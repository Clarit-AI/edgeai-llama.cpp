...
- If you build the image on a different machine, pass `--build-arg GGML_NATIVE=OFF` to the build command (default is `ON` for native optimizations). Example: `podman image build --build-arg GGML_NATIVE=OFF --format Dockerfile --file ik_llama-cpu.Containerfile --target full --tag ik_llama-cpu:full`.
